...
import { Mic, MicOff, PhoneOff, Phone, Volume2, Monitor, AlertCircle } from 'lucide-react';
...
function VolumeIndicator({ stream, isMuted }: { stream: MediaStream | null, isMuted?: boolean }) {
  ...
  return (
    <div className="flex items-center space-x-[2px] h-3 w-8">
      {[...Array(4)].map((_, i) => (
        <motion.div
          key={i}
          animate={{ 
...
export function VoiceRoom({ channelId }: { channelId: string }) {
  ...
  // Helper to update voice users with their current stream
  ...
  return (
    <div className="bg-[#1E1F22] border border-white/5 rounded-xl p-4 mb-4 shadow-xl">
      <div className="flex items-center justify-between mb-4">
        ...
        {!inVoice ? (
          <button
            onClick={joinVoice}
            className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center shadow-lg shadow-emerald-500/20"
          >
          ...
          <div className="flex items-center space-x-2">
             <button
              ...
              className={`p-2 rounded-md transition-colors ${isSharingScreen ? 'bg-blue-500/20 text-blue-400' : 'bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white'}`}
            >
              <Monitor className="w-4 h-4" />
            </button>
            <button
              onClick={toggleMute}
              className={`p-2 rounded-md transition-colors ${isMuted ? 'bg-red-500/20 text-red-400' : 'bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white'}`}
            >
              {isMuted ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4" />}
            </button>
            <button
              onClick={leaveVoice}
              className="bg-red-500 hover:bg-red-600 text-white px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center shadow-lg shadow-red-500/20"
            >
      ...
      {error && (
        <div className="flex items-center p-2 mb-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-[10px]">
        ...
        {inVoice && (
          ...
          >
            <div className="grid grid-cols-1 gap-2">
              {voiceUsers.map((vu) => (
                <div key={vu.socketId} className="flex items-center justify-between bg-white/5 rounded-lg p-2 border border-white/5">
                  <div className="flex items-center min-w-0">
                    <div className="relative w-8 h-8 rounded-full overflow-hidden mr-3 border border-white/10">
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className="text-xs font-bold text-white truncate">{vu.name || 'Unknown'}</span>
                      <VolumeIndicator 
                        stream={vu.stream} 
                        isMuted={vu.isMuted}
                      />
            ...
            {voiceUsers.length === 1 && (
              <p className="text-[10px] text-zinc-500 italic text-center py-2">Waiting for others to join...</p>
            )}
          </motion.div>
        )}
      </AnimatePresence>
